...
class SummarizedModel(PowerBaseModel):
    ...
    def build_model(self, n_features, n_targets, n_targets_edge, n_edge_features, n_hiddens, n_layers, batchnorm,
                    residuals):
        self.encode_layer = self._build_encode_layer(sum(n_features.values()), n_hiddens)
        ...
        self.gen_output_layer = self.build_output_layer(n_hiddens + n_features["gen"], n_hiddens, n_targets["gen"])
    ...
    def forward(self, data):
        ...
        gen_as_bus = scatter_add(data.gen, data.gen_to_bus, dim=0, dim_size=data.bus.shape[0])
        load_as_bus = scatter_add(data.load, data.load_to_bus, dim=0, dim_size=data.bus.shape[0])
        shunt_as_bus = scatter_add(data.shunt, data.shunt_to_bus, dim=0, dim_size=data.bus.shape[0])

        x = torch.cat([data.bus, gen_as_bus, load_as_bus, shunt_as_bus], dim=1)
        ...
        bus = x
        gen = torch.cat([x[data.gen_to_bus], data.gen], dim=1)
